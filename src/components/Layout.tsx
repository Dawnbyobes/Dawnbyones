      ...
      <header className="sticky top-0 z-50 bg-[#0a0a0a]/95 backdrop-blur border-b border-[#1a1a1a]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14">
            <Link to="/" className="text-white text-sm font-medium tracking-wide hover:opacity-80 transition-opacity">
              冰箱里的世界
            </Link>

            <nav className="hidden sm:flex items-center gap-6">
              <Link to="/novels" className="text-[#888] text-sm hover:text-white transition-colors">全部作品</Link>
              {isAdmin && (
          ...
          {mobileOpen && (
            <div className="sm:hidden py-4 border-t border-[#1a1a1a] space-y-3">
              <Link to="/novels" onClick={() => setMobileOpen(false)} className="block text-[#888] text-sm hover:text-white py-1">全部作品</Link>
